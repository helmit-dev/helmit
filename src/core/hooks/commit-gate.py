...
if MSG and not MSG.startswith(("Merge ", "Revert ")):
    for reference in re.findall(r"\[[^\]\n]+\]", MSG):
        if re.search(r"(?:satisfies|supports|covers):\s*REQ-|^\[CHG-", reference):
            if not (TASK_REF.search(reference) or CHG_REF.fullmatch(reference)):
                print("HelmIt commit gate BLOCKED: malformed task reference. Use [<phase>.<task> satisfies: REQ-xxx] or [CHG-NNN.1].", file=sys.stderr)
                raise SystemExit(2)
...
if MSG and not MSG.startswith(("Merge ", "Revert ")):
    for path in staged_blobs():
        if not (re.fullmatch(r"\.helmit/phases/[^/]+/CHART\.md", path)
                or path in ("docs/PLAN.md", ".helmit/CHANGES.md")):
            continue
        artifact = Path(PROJ) / path
        before, after = git_blob("HEAD", artifact), git_blob("", artifact)
        if path == ".helmit/CHANGES.md":
            closed = re.findall(r"^\| (CHG-[0-9]+) \|.*\|\s*done\s*\|$", after, re.M)
            previous = set(re.findall(r"^\| (CHG-[0-9]+) \|.*\|\s*done\s*\|$", before, re.M))
            newly_closed = [identity + ".1" for identity in closed if identity not in previous]
        else:
            closed = re.findall(r"^- \[[xX]\]\s+(?:\*\*)?([A-Za-z0-9_-]+\.[0-9]+)(?=\s|—)", after, re.M)
            newly_closed = [task for task in closed if task_mark(before, task) != "x"]
        if any(task not in task_ids for task in newly_closed):
            print("HelmIt commit gate BLOCKED: task closure requires its explicit identity and proof.", file=sys.stderr)
            raise SystemExit(2)
