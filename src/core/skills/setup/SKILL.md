...
- Clear, localized, directly verifiable work uses ordinary edits and Git,
  including source labels and known-cause fixes. No CHG or task ID is required.
  Use CHG when investigation, decisions, coordination or continuity need a
  record; use deliveries for larger commitments. Judge impact, not file paths.
<!-- helmit:end -->
```
