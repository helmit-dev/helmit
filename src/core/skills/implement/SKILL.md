...
## Direct work before lifecycle entry

For an authorized request whose intent and solution are clear, impact is local,
and verification is direct, edit and check the result using ordinary Git.
This includes labels in source, spacing, cache ignores, and bugs with a known
cause. Choose by meaning, uncertainty and impact, never file path or line count.
No new command, CHG, task ID, claim, lock ceremony, ledger, proof worktree or
STATE transition is required for this route. Inspect relevant diffs and preserve
unrelated work; the normal commit hooks and project authorization rules apply.
Use the smallest meaningful check; do not manufacture tests for a text edit.
Do not call task-only transaction/checkpoint writers with a fabricated identity.
Report the result briefly. Native activity hooks may record activity without a
task; do not attribute direct work to an unrelated open claim.

Work belonging to an existing task keeps its task identity and proof. If new
investigation, decisions, coordination or continuity need a durable record, use
CHG for bounded corrections or a delivery for a larger product commitment.
An explicit request to maintain a structured record is honored. Merely having
an open phase does not turn unrelated direct work into phase work.
The lifecycle preconditions and process below apply to that tracked work.
...
Every commit belonging to a registered task carries its stable task and requirement
